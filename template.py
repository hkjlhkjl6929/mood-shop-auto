...
<script>
(function () {{
  var sections = ['overview', 'roi', 'fee', 'card', 'total'];
  var section = 'overview';
  try {{
    var saved = JSON.parse(localStorage.getItem('moodShopState') || '{{}}');
    if (sections.indexOf(saved.section) !== -1) section = saved.section;
  }} catch (e) {{}}
  document.documentElement.setAttribute('data-initial-section', section);
}})();
</script>
...
<style>
  * {{ box-sizing: border-box; }}
  ...
  html[data-initial-section] .sb-tab.active {{ background: transparent; color: #4a4438; box-shadow: none; font-weight: 500; }}
  html[data-initial-section="overview"] .sb-tab[data-section="overview"],
  html[data-initial-section="roi"] .sb-tab[data-section="roi"],
  html[data-initial-section="fee"] .sb-tab[data-section="fee"],
  html[data-initial-section="card"] .sb-tab[data-section="card"],
  html[data-initial-section="total"] .sb-tab[data-section="total"] {{ background: #fff; color: #1a1a1a; box-shadow: 0 1px 4px rgba(0,0,0,0.05); font-weight: 700; }}
  ...
  html[data-initial-section] .tab-content.active {{ display: none; }}
  html[data-initial-section="overview"] #sec-overview,
  html[data-initial-section="roi"] #sec-roi,
  html[data-initial-section="fee"] #sec-fee,
  html[data-initial-section="card"] #sec-card,
  html[data-initial-section="total"] #sec-total {{ display: block; }}
  ...
  .screenshot-head {{ display: flex; align-items: center; justify-content: space-between; gap: 14px; }}
  .screenshot-head h2 {{ margin-bottom: 4px; }}
  .screenshot-note {{ font-size: 12px; color: #8a8170; margin-top: 4px; }}
  .shot-btn {{ border: 1px solid #d97706; background: #fff7ed; color: #9a3412; border-radius: 8px; padding: 9px 13px; font-size: 13px; font-weight: 700; cursor: pointer; }}
  .shot-btn:hover {{ background: #ffedd5; }}
  .shot-modal {{ position: fixed; inset: 0; z-index: 9999; display: none; align-items: center; justify-content: center; padding: 20px; background: rgba(17, 24, 39, 0.72); }}
  .shot-modal.open {{ display: flex; }}
  .shot-panel {{ position: relative; max-width: min(96vw, 1280px); max-height: 92vh; background: #fff; border-radius: 10px; overflow: hidden; box-shadow: 0 20px 60px rgba(0,0,0,0.32); }}
  .shot-panel img {{ display: block; max-width: 96vw; max-height: 92vh; object-fit: contain; }}
  .shot-close {{ position: absolute; top: 10px; right: 10px; border: 0; background: rgba(17,24,39,0.78); color: #fff; border-radius: 999px; width: 34px; height: 34px; font-size: 20px; line-height: 34px; cursor: pointer; }}
...
const screenshotUrl = month => `screenshots/${{APP.year}}-${{pad2(month)}}.png`;
...
function openScreenshotModal(url) {{
  ...
  if (!modal) {{
    document.body.insertAdjacentHTML('beforeend', `
      <div class="shot-modal" id="screenshot-modal" aria-hidden="true">
        <div class="shot-panel">
          <button class="shot-close" type="button" data-close-shot aria-label="關閉">×</button>
          <img alt="Meta 刷卡截圖">
        </div>
      </div>
    `);
    ...
    modal.querySelector('[data-close-shot]').addEventListener('click', closeScreenshotModal);
    document.addEventListener('keydown', e => {{
      if (e.key === 'Escape') closeScreenshotModal();
    }});
  }}
  modal.querySelector('img').src = url + '?v=' + Date.now();
  modal.classList.add('open');
  modal.setAttribute('aria-hidden', 'false');
  document.body.style.overflow = 'hidden';
}}

function wireScreenshotButton(url) {{
  const box = document.getElementById('card-screenshot-box');
  if (!box) return;

  const probe = new Image();
  probe.onload = () => {{
    box.style.display = '';
    const btn = box.querySelector('[data-shot]');
    if (btn) btn.addEventListener('click', () => openScreenshotModal(url));
  }};
  probe.onerror = () => box.remove();
  ...
  const screenshotCard = shotUrl ? `
    <div class="card screenshot-card" id="card-screenshot-box">
      <div class="screenshot-head">
        <div>
          <h2>Meta 刷卡截圖</h2>
          <div class="screenshot-note">${{APP.year}}-${{pad2(state.month)}}.png</div>
        </div>
