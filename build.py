...
def main():
    ...
    # Copy uploaded Meta card screenshots into the GitHub Pages artifact.
    screenshots_src = "screenshots"
    screenshots_dst = os.path.join(OUT_DIR, "screenshots")
    if os.path.isdir(screenshots_src):
        if os.path.isdir(screenshots_dst):
            shutil.rmtree(screenshots_dst)
        shutil.copytree(screenshots_src, screenshots_dst)
        print("[OK] screenshots/ copied to site/screenshots/")
    else:
        print("[INFO] screenshots/ not found, skip")
